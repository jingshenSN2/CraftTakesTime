...
public class CraftManager {
    ...
    private ContainerScreen currentGuiContainer;
    ...
    public boolean initCraft(ContainerScreen gui, int invSlot) {
        ...
        if (this.currentGuiContainer.getContainer().getSlot(outputSlot).getStack().isEmpty()) {
            return false;
        }
        ...
        if (!isCrafting()) {
            craftPeriod = getCraftingTime(
                    this.currentGuiContainer.getContainer(), outputSlot, properties.getIngredientSlots(), properties);
    ...
    public void tick() {
        ...
        if (this.isCrafting()) {
            ...
            ItemStack resultStack = this.currentGuiContainer.getContainer()
                    .inventorySlots.get(outputSlot).getStack();
            ...
            if (this.getCurrentCraftTime() < this.getCraftPeriod()) {
                ...
                List<Item> oldRecipe = getIngredientItems(
                        this.getCurrentGuiContainer().getContainer(), ingredientSlots);

                ((ITimeCraftGuiContainer) this.currentGuiContainer).handleCraftFinished(
                        this.getCurrentGuiContainer().getContainer().getSlot(outputSlot), outputSlot);
